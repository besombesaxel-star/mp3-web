...
export default function AccountPage() {
  ...
  useEffect(() => {
    ...
    async function loadAccountData() {
      ...
      try {
        ...
        if (cancelled) return;
        setFavoriteCount(0); setPublicBio(""); setUploads([]); setTotalUploads(0);
      } finally {
        if (!cancelled) setProfileLoading(false);
      }
    }
    if (!isAuthenticated || !accessToken) {
      setFavoriteCount(0); setPublicBio(""); setUploads([]); setTotalUploads(0); setProfileLoading(false); return;
    }
  ...
  async function handlePasswordUpdate(e: React.FormEvent) {
    ...
    catch (e) { setPasswordMsg({ text: getErrorMessage(e, "Mise à jour du mot de passe impossible."), ok: false }); }
    finally { setBusy(""); }
  }
          ...
          <div className="space-y-4">
            ...
            <section className="rounded-3xl border border-white/8 bg-white/[0.04] p-6 mp3-fade-up">
              <div className="flex items-center gap-4">
                <div className={`h-14 w-14 shrink-0 rounded-2xl bg-gradient-to-br ${gradient} flex items-center justify-center text-lg font-semibold text-white shadow-lg`}>
                  {initials}
                </div>
                <div className="min-w-0 flex-1">
                  ...
                  {createdAtLabel && (
                    <p className="text-xs text-white/25 mt-1">Membre depuis le {createdAtLabel}</p>
                  )}
                </div>
            ...
            {/* Stats */}
            <div className="grid grid-cols-3 gap-3 mp3-fade-up" style={{ animationDelay: "60ms" }}>
            ...
            <section className="rounded-3xl border border-white/8 bg-white/[0.04] p-6 mp3-fade-up" style={{ animationDelay: "200ms" }}>
              <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-2">
                  ...
                  {totalUploads > 0 && (
                    <span className="text-xs text-white/25 tabular-nums">{totalUploads}</span>
                  )}
                </div>
              ...
              {profileLoading ? (
                ...
                <div className="rounded-2xl border border-white/8 bg-white/3 px-4 py-7 text-center">
                  ...
                  <Link
                    href="/upload"
                    className="inline-flex items-center gap-1.5 h-8 px-4 rounded-full bg-white/8 border border-white/10 text-xs text-white/60 hover:bg-white/12 hover:text-white/90 transition"
                  >
                    <Upload size={11} />
                    Ajouter un son
                  </Link>
          ...
          <div className="rounded-3xl border border-white/8 bg-white/[0.04] p-6 mp3-fade-up">
            ...
            {mode === "signin" ? (
              <form className="space-y-4" onSubmit={handleSignIn}>
                ...
                {authMsg && (
                  <p className={["text-xs", authMsg.ok ? "text-green-400/90" : "text-red-400/90"].join(" ")}>
                    {authMsg.text}
                  </p>
                )}
