...
export async function POST(req: Request) {
  ...
  const rateLimit = checkRateLimit(`chat:${auth.user.id}`, CHAT_LIMIT, CHAT_WINDOW_MS);
  if (!rateLimit.allowed) {
    return NextResponse.json({ ok: false, error: "Tu envoies trop de messages, patiente un instant." }, { status: 429 });
  }
